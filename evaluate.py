"""
evaluate.py — unified evaluation for all five architectures.
...
Metrics:
  ...
  (* read back from results/metrics_log.csv)

Multi-seed: pass several checkpoints (one per seed) to report mean ± std.
Significance: paired bootstrap (n=1000, seed=123) Arch 5 vs Arch 4 on BLEU. We
roll our own bootstrap rather than use a sacrebleu class whose name has changed
across versions.

Examples:
  python evaluate.py --ckpts results/arch1_gru_seed191_best.pt
  python evaluate.py --ckpts a191.pt a1604.pt a4090.pt --beam 5
  python evaluate.py --significance arch4_seed191_best.pt arch5_seed191_best.pt
"""
