...
Shared settings (tokenizer, data, seeds, special-token ids) are top-level
constants. Per-architecture hyperparameters live in ARCH_CONFIGS[<name>].

Usage:
    from config import CONFIG, get_arch_config, SMOKE
    cfg = get_arch_config("arch1_gru")

The "smoke" profile (tiny data, tiny model, 1-2 epochs) runs the whole pipeline
end-to-end as a quick check before a long run. Turn it on with the env var
SPEECHBRIDGE_SMOKE=1 or `--smoke`.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
SEED = 123
EVAL_SEEDS = [191, 1604, 4090]  # mean ± std over 3 seeds for final reporting

# Special-token ids — MUST match data/train_tokenizer.py exactly.
PAD_ID, UNK_ID, BOS_ID, EOS_ID = 0, 1, 2, 3
LANG_TOKENS = ["<zh>", "<en>"]  # user-defined symbols; prepended to SOURCE side

VOCAB_SIZE = 32000
CHAR_COVERAGE = 0.9999
MODEL_TYPE = "bpe"
MAX_TOKENS_PER_SIDE = 100  # length filter upper bound
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset configuration (see README for sources)
# ─────────────────────────────────────────────────────────────────────────────
DATA = {
    ...
    "opensubtitles": {
        ...
        "version": "v2018",
        "l1": "en",
        "l2": "zh_cn",  # OPUS uses zh_cn / zh_tw, not bare "zh"
        "sample": 800_000,  # subsample target (~800K)
    },
    ...
    "wmt_testset": "wmt19",         # newstest2019 (has both zh-en and en-zh)
    "wmt_testset_alt": "wmt20",     # newstest2020
    # FLORES+ (eval only). openlanguagedata/flores_plus, devtest split.
    "flores": {
        "hf_id": "openlanguagedata/flores_plus",   # GATED: requires HF login/token
        "split": "devtest",   # dev=997, devtest=1012
        # NOTE: FLORES+ (OLDI) uses the Mandarin code 'cmn_Hans', NOT the old
        # facebook/flores 'zho_Hans'. (cmn = Mandarin; zho = macrolanguage.)
        "zh_code": "cmn_Hans",
        "en_code": "eng_Latn",
    },
...
# ─────────────────────────────────────────────────────────────────────────────
EPOCH_CAP = 200             # safety guard only; early stopping is the real terminator
EARLY_STOP_PATIENCE = 10    # stop after dev loss stalls this many epochs
...
class ArchConfig:
    ...
    grad_clip: float = 1.0
    amp_safe: bool = True             # use fp16 mixed precision on CUDA. ConvS2S sets
                                      # this False: its stacked GLU/residual blocks
                                      # overflow fp16 (-> NaN), so it trains in fp32.
    # batching
    batch_size: int = 128
    ...
    # loop
    epochs: int = EPOCH_CAP           # SAFETY CAP only — early stopping ends training
    patience: int = EARLY_STOP_PATIENCE   # stop after dev loss stalls this many epochs
    teacher_forcing: bool = False     # scheduled-sampling for RNNs
    tf_start: float = 1.0
    tf_end: float = 0.5
    tf_decay_epochs: int = 20         # reach tf_end by this epoch, then hold
                                      # (decoupled from total epochs, which are now open-ended)
    # inference
    beam_size: int = 5
...
ARCH_CONFIGS: dict[str, ArchConfig] = {
    ...
    "arch3_convs2s": ArchConfig(
        ...
        # ConvS2S (Gehring et al.) used NAG. lr 0.25 / momentum 0.99 are NAG
        # settings; lr 0.25 diverges under Adam-family optimizers. nadam_momentum
        # (0.99) is reused as the SGD momentum.
...
# Hypothesis (Arch 5 vs Arch 4), encoded as data for the report:
HYPOTHESIS = (
    "Arch 5 (relative PE + label smoothing) should beat Arch 4 by >= 0.5 BLEU "
    "on BOTH directions (ZH->EN and EN->ZH) without extra compute."
)
